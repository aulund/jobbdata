...
class VariantInfoCollector:
    ...
    def create_widgets(self):
        ...
        self.button_back.grid(column=0, row=12, columnspan=2, padx=10, pady=5)
    ...
    def autofill_gene_data(self, event):
        ...
        # Store in the self.data for later use
    ...
    def submit_all(self):
        ...
        # Ensure LID-NR is in the data
        if "LID-NR" not in self.data:
            self.data["LID-NR"] = self.general_info["LID-NR"]

        # Ensure that Proband, Genotype, Phenotype, and Sequencing method are passed on
        ...
        # Log the data before passing it to generate_document
        print(f"Data before generating document: {self.data}")

        output_directory = r"U:\DNA_Sekvenseringsresultat\Remissvar Koagulation\Väntande på att svaras ut"  # Replace with the actual path you want to use
