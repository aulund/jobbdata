...
def generate_document(data):
    """Generates a genetic report document based on the collected data."""
    doc = Document()

    # Add separator and 10 spaces before each paragraph
    separator = "=" * 40
    spaces = ' ' * 10
    ...
    for i, variant in enumerate(data['variants'], 1):
        ...
        p = doc.add_paragraph()
        run = p.add_run(f"** Genetisk Variant {i}: **")
        run.bold = True
        run.font.size = 100000  # adjust size as needed
        variant_info = process_variant_info(variant, spaces)
        doc.add_paragraph(spaces + variant_info)

        doc.add_paragraph(separator)
        p = doc.add_paragraph()
        run = p.add_run(f"** Bedömning och Databasreferenser för Variant {i}: **")
        run.bold = True
        run.font.size = 100000  # adjust size as needed
        doc.add_paragraph(spaces + f"Bedömning enligt ACMG-kriterierna: {variant['ACMG criteria assessment']}. (Varianten anses inte ha någon klinisk signifikans baserat på befintliga bevis.)")
        doc.add_paragraph(spaces + f"Tidigare rapporter i ClinVar och hemofilidatabaserna: {variant['ClinVar and hemophilia database reports']}.")

        if 'Inversions' in variant and variant['Inversions']:
            doc.add_paragraph(separator)
            p = doc.add_paragraph()
            ...
            if variant['Inversions'] == 'ja':
                doc.add_paragraph(spaces + f"Resultat av inversionerna i intron 1 och 22: {variant.get('Inversion result', 'Inga resultat tillgängliga')}.")
            elif variant['Inversions'] == 'nej':
                doc.add_paragraph(spaces + f"Utredning av inversionsvarianter i intron 1 och 22 utfördes inte på grund av: {variant.get('Inversion reason', 'Ingen anledning angiven')}.")

        # CSV data förberedelse
        if variant['Further studies'] == 'ja':
            csv_row = prepare_csv_row(data['LID-NR'], variant)
    ...
    if data['Sequencing method'] == 'mps':
        ...
        doc.add_paragraph(spaces + f"Exon {data['Exon']} av {data['variants'][0]['Gene'].upper()} har analyserats med Sangersekvensering.")
    ...
    doc.add_paragraph(separator)
    p = doc.add_paragraph()
    run = p.add_run("** För frågor eller ytterligare information, vänligen kontakta: **")
    run.bold = True
    run.font.size = 100000  # adjust size as needed
    doc.add_paragraph(spaces + "Professor, Överläkare: Jovan Antovic")
    doc.add_paragraph(spaces + "Sjukhuskemist: August Lundholm")
    ...
    # Spara dokument
    save_path = os.path.join('U:', 'DNA_Sekvenseringsresultat', 'Remissvar Koagulation', 'Väntande på att svaras ut', f"{data['LID-NR']} ({data['variants'][0]['Gene'].upper()})", 'genetisk_rapport.docx')
    ...
    if csv_data_rows:
        csv_path = 'U:\\DNA_Sekvenseringsresultat\\Remissvar Koagulation\\intressanta_varianter.csv'
